...
> 记录日期：2026-09-03
> 证据范围：Phase 2 / Task 2.1 环境与最小推理，以及第 11 节单图探索测试；不代表已完成模型质量基线或最终选型。新 session 的复核说明见第 12 节；执行范围以用户当前指令及 current.md/task.md 为准。
...
## 12. 新 session 的环境复核与既有记录使用

- 同一工作目录继续时，先检查已有解释器、依赖、四张原图与本地模型目录；不重复执行 uv init、创建 Git 仓库或下载已有权重。新目录/机器不自带 .venv、样图、参考或模型缓存，需分别准备。
- 依赖需要同步时沿用第 10 节的锁定命令；先确认当前解释器为 Python 3.12.13。环境报告使用新的独立输出目录，避免覆盖旧记录。已知的历史 CPU 结果不代替新环境的设备检查。
- 目录存在只证明路径可访问；正式推理继续核对模型配置、权重及文件 manifest。模型路径以实际文件为准，文档中的本机缓存位置是定位线索，不应写死进脚本。
- 第 10 节三条命令是 Task 2.1 的 smoke 示例：文字输出上限 32、视觉输出上限 64，且现有 Qwen3-VL 提示词要求简短回答。2.2.3 工具补齐前不能把这些命令直接当作四图完整基线，也不要提前运行尚不存在的 run_baseline.py/evaluate_baseline.py/correct_text.py。
- 第 6–10 节中的部分旧验证产物已按用户要求清理；保留为历史记录，不要求恢复这些旧文件才启动 2.2。当前是否还有 fuzzy-photo 的本地产物须现场检查，正式基线生成独立的新记录。
- 若 shell/文件工具在执行命令前报 sandbox setup/helper 错误，应记录为工具环境问题，按实际权限机制处理；不能当成依赖或模型失败，也不为此改变项目配置。
- 缺少样图时仍可实现合成评分/失败处理测试；缺少模型时仍可准备参考草稿和工具。只有依赖实际缺失资源的步骤保留未完成，并集中报告需要用户提供的输入。
